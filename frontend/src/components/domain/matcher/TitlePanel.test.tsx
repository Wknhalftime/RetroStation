...
describe('TitlePanel tier + score formatting', () => {
  it('shows both tier and score joined on "·" when both are present', () => {
    ...
    expect(
      screen.getByText(/Tier: musicbrainz_id_exact · Score: 92%/),
    ).toBeDefined()
  })
  ...
  it('shows score only, no leading separator, when tier is absent', () => {
    ...
    render(
      <TitlePanel
        artist={makeArtist([
          makeIdentity({ match_tier: null, confidence_score: 92 }),
        ])}
        onFileSearch={vi.fn()}
      />,
      { wrapper: wrapperFor(makeClient()) },
  ...
  it('renders no tier/score line when both are absent', () => {
    const { container } = render(
      <TitlePanel
        artist={makeArtist([
          makeIdentity({ match_tier: null, confidence_score: null }),
        ])}
        onFileSearch={vi.fn()}
      />,
      { wrapper: wrapperFor(makeClient()) },
...
describe('TitlePanel reason badge', () => {
  it('prefers reason_detail over reason_code', () => {
    ...
    expect(
      screen.getByText(/Score 72% — below confidence threshold/),
    ).toBeDefined()
...
describe('TitlePanel artist gate', () => {
  it('shows placeholder when artist is null', () => {
    render(
      <TitlePanel artist={null} onFileSearch={vi.fn()} />,
      { wrapper: wrapperFor(makeClient()) },
    )
  ...
  it('shows "resolve artist first" when artist is unresolved', () => {
    ...
    render(
      <TitlePanel artist={artist} onFileSearch={vi.fn()} />,
      { wrapper: wrapperFor(makeClient()) },
    )
