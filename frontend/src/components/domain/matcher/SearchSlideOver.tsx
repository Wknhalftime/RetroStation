...
export function SearchSlideOver({
  ...
  const mbSearch = useMbArtistSearch(open && mode === "mb-artist" ? debouncedQuery : "");
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2">
          {mode === "mb-artist" ? (
            <>
              ...
              {mbSearch.isError && !mbSearch.isLoading && (
                <p className="px-1 py-4 text-sm text-red-500">
                  {mbSearch.error instanceof Error ? mbSearch.error.message : "Search failed"}
                </p>
              )}
                ...
                <div className="space-y-0.5">
                  {mbResults.map((item) => (
                    ...
                    >
                      ...
                      {item.disambiguation && (
                        <p className="truncate text-xs text-gray-400">{item.disambiguation}</p>
                      )}
            ...
            <>
              ...
              {error && !loading && <p className="px-1 py-4 text-sm text-red-500">{error}</p>}
