...
function artistWithCandidates(candidates: QueueArtist["candidates"]): QueueArtist {
  return {
...
describe("firstCandidateMbid", () => {
  ...
  it("returns null when mbid is missing", () => {
    expect(firstCandidateMbid(artistWithCandidates([{ name: "Prince" }]))).toBeNull();
  });

  it("returns null when mbid is not a string (defensive)", () => {
    expect(firstCandidateMbid(artistWithCandidates([{ mbid: 42 }]))).toBeNull();
  });
